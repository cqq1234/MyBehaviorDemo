...
public class AnimHelper {
    ...
    public static void translateLeft(View view,ViewPropertyAnimatorListener listener){
        ViewCompat.animate(view).translationX(0).setDuration(DURATION).setListener(listener).setInterpolator(INTERPOLATOR).withLayer().start();
    }

    public static void translateRight(View view, ViewPropertyAnimatorListener listener){
        int width = view.getWidth();
        ViewGroup.LayoutParams params = view.getLayoutParams();
        ViewGroup.MarginLayoutParams layoutParams = params instanceof ViewGroup.MarginLayoutParams ? ((ViewGroup.MarginLayoutParams) params) : null;
        if (layoutParams != null) width += layoutParams.rightMargin;
        ViewCompat.animate(view).translationX(width).setDuration(DURATION).setListener(listener).setInterpolator(INTERPOLATOR).withLayer().start();
    }
